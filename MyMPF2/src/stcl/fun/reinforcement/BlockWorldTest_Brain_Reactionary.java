...
public class BlockWorldTest_Brain_Reactionary {
	...
	public static void main(String[] args){
		BlockWorldTest_Brain_Reactionary bwt = new BlockWorldTest_Brain_Reactionary();
		bwt.setup(4);
		bwt.run(1000);
	}
	...
	public void run(int numEpisodes){
		for (int i = 1; i <= numEpisodes; i++){
			agent.newEpisode();
			int steps = runEpisode(agent, 1 - (double) i / numEpisodes);
	...
	public int runEpisode(Network agent, double explorationChance){
		...
		do{
			...
			loadNetwork(agent, state, action);
			agent.step(reward);	
			action = getAction(agent);
			
			state = move(state, ACTIONS.values()[action]);
			
			if (prediction != null){
				if (!prediction.equals(state)) errors++;
			}
	...
	private class State{
		...
		private BlockWorldTest_Brain_Reactionary getOuterType() {
			return BlockWorldTest_Brain_Reactionary.this;
		}
